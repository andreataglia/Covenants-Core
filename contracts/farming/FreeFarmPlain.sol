...
contract FarmMain is IFarmMain, ERC1155Receiver {
    ...
    function addLiquidity(uint256 positionId, FarmingPositionRequest memory request) public override payable activeSetupOnly(request.setupIndex) byPositionOwner(positionId) {
        ...
        farmingPosition.reward = calculateFarmingReward(positionId, false);
    ...
    function withdrawReward(uint256 positionId) public byPositionOwner(positionId) {
        ...
        FarmingPosition storage farmingPosition = _positions[positionId];
        // rebalance setup
        uint256 currentBlock = currentBlock > _setups[farmingPosition.setupIndex].endBlock ? _setups[farmingPosition.setupIndex].endBlock : block.number;
        ...
        uint256 reward = calculateFarmingReward(positionId, false);
        _rewardPerTokenPaid[positionId] = _rewardPerTokenPerSetup[farmingPosition.setupIndex];
    ...
    function calculateFarmingReward(uint256 positionId, bool isExt) public view returns(uint256 reward) {
        FarmingPosition memory farmingPosition = _positions[positionId];
     ...
     */
    function _removeLiquidity(uint256 positionId, uint256 setupIndex, bool unwrapPair, uint256 removedLiquidity) private {
        ...
        if (farmingPosition.creationBlock != 0 && positionId != 0) {
